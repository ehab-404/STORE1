    ...
    public class AuditSaveChangesInterceptor : SaveChangesInterceptor
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            foreach (var entry in entries)
            {
                ...
                var pkname = entry.Properties.FirstOrDefault(p => p.Metadata.IsPrimaryKey()).Metadata.Name;
                ...
                if (entry.State == EntityState.Added)
                {
                    ...
                    var sql = $"SELECT COALESCE(MAX([{pkname}]), 0) + IDENT_INCR('{tablename}') FROM [{tablename}]";
                ...
                var objz = JsonSerializer.Deserialize<Dictionary<string, object>>(json);
                objz["Id"] = primarykey;
                json = JsonSerializer.Serialize(objz);
